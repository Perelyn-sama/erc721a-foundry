...
contract CustomNFT is Ownable, ERC721A, ReentrancyGuard {
    ...
    address private fundWallet =
        payable(0x68df7639ef63fA25D618cdEEBbdd1093d37c8e18); // Owner Fund Wallet Address
    address private devOneWallet =
        payable(0x476E35cc46f86B3028Ce0c2bD0C8496b6e79f47f); // Developer One Wallet Address
    address private devTwoWallet =
        payable(0xfC19Fe6137631274E3aBE50028444da0B94EBe73); // Developer Two Wallet Address
    ...
    constructor(uint256 maxBatchSize_, uint256 collectionSize_)
        ERC721A("Custom NFT", "CNFT", maxBatchSize_, collectionSize_)
    {
    ...
    function whitelistMint() external payable callerIsUser saleIsOpen {
        ...
        require(
            ...
            "WHITELIST_SALE: not eligible for whitelist mint"
        );
        require(
            totalSupply() + 1 <= collectionSize,
            "WHITELIST_SALE: reached max supply"
        );
    ...
    function publicSaleMint(uint256 quantity, uint256 callerMintKey)
        external
        payable
        callerIsUser
        saleIsOpen
    {
        ...
        require(
            mintKey == callerMintKey,
            "PUBLIC_SALE: called with incorrect public sale key"
        );

        require(
            isPublicSaleOn(publicPrice, mintKey, publicSaleStartTime),
            "PUBLIC_SALE: public sale has not begun yet"
        );
        require(
            totalSupply() + quantity <= collectionSize,
            "PUBLIC_SALE: reached max supply"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPublicBatchSize,
            "PUBLIC_SALE: can not mint this many"
        );
